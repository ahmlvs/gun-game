...
from config import WHITE, BLACK, RED, SCREEN_SIZE, FPS
...
class Cannon(GameObject):
    ...
    def __init__(self, coord=[30, SCREEN_SIZE[1]//2], angle=0,
        ...
        '''
        Constructor method.
        Sets coordinate, direction, min and max power and color of the gun.
        '''
        self.coord = coord
        self.angle = angle
...
class MovingTarget(Target):
    def __init__(self, coord=None, color=None, rad=30):
        ...
        self.vel = [0, 0]
        self.vel[0] = randint(-5, +5)
        self.vel[1] = randint(-5, +5)
    ...
    def check_corners(self, refl_ort=0.8, refl_par=0.9):
        '''
        Reflects target's velocity when target bumps into the screen corners.
        Implemetns inelastic rebounce.
        '''
        for i in range(2):
            if self.coord[i] < self.rad:
